...
- Vercel runs `cd Spaceship && npm ci && npm run build && cd .. && node scripts/prerender.js` on every deploy (see `vercel.json`).
...
The site is fully crawlable by AI bots and search engines. The build step
pre-renders content into the static HTML so crawlers that don't execute JS
still see all essays, book reviews, and research nodes.

- `scripts/prerender.js` — runs at build time. Inlines essay/book/research
  content into the relevant HTML pages between
  `<!-- PRERENDER:KEY:START/END -->` sentinels, and generates one static HTML
  page per essay and per book review under `essays/<slug>.html` and
  `books/<slug>.html` for direct crawler access. It also regenerates `llms.txt`
  and validates that DONE research links resolve.
- `robots.txt` — allows all major crawlers (GPTBot, ChatGPT-User, anthropic-ai,
  ClaudeBot, PerplexityBot, Googlebot, Applebot, …); disallows `/Spaceship/`.
- `sitemap.xml` — canonical URL list (apex host `valerianteissier.com`).
- `llms.txt` — structured content inventory at the root, per llmstxt.org.
- `vercel.json` rewrites — clean URLs (`/essays/slug` → `essay-detail.html?essay=slug`).
  Canonical host is the **apex** (`valerianteissier.com`); keep canonical link
  tags aligned with the serving host.

**Adding content:** just edit the JSON registries and drop the markdown file as
described below — the prerender script picks it up at build time, no manual
HTML generation needed. **Adding a new HTML page:** also add an `ensureAnchor`
+ `patchSection` block in `scripts/prerender.js` so its content gets inlined.
